...
vascr_plot_line = function(data, priority = NULL, error = Inf, alpha = 0.1, ...)
{
  ...
  if (replication == "wells") {
    ...
    if (length(priority) == 1) {
      ...
      plot = ggplot(data = data, aes_string(x = xaxis, y = yaxis, colour = priority[2],fill = priority[2], ymin = "ymin", ymax = "ymax")) + geom_line()
      
    }else if (length(priority)>2) {
      
      plot = ggplot(data = data, aes_string(x = xaxis, y = yaxis, colour = priority[2], linetype = priority[3], fill = priority[2], ymin = "ymin", ymax = "ymax")) + geom_line()
    }
  }
