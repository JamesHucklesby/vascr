...
hour5 = hour5 + scale_shape_manual(values = c(16,3,1),labels = c("Media Only",expression(paste("62,500 cells/",cm^2)),expression(paste("250,000 cells/",cm^2))))+ theme(legend.text.align = 0) + labs(Sample = "HCMVEC Seeding Density")

hour47 = hour47 + scale_shape_manual(values = c(16,3,1), labels = c("Media Only",expression(paste("62,500 cells/",cm^2)),expression(paste("250,000 cells/",cm^2))))+ theme(legend.text.align = 0) + labs(Sample = "HCMVEC Seeding Density")
...
p8 = p8 +theme(legend.position = "none")+ ylab(vascr_titles("R", 4000)) + xlab("Time (hours)")
...
make_multi_ccf = function(mini1, tomake)
{
 ...
 pcombinations$sample = str_replace_all(pcombinations$sample, "IL1b", "IL1\U03B2")
  ...
  barplot = ggplot(pcombinations, aes(x = sample, y = mean, ymax = mean+sem, ymin = mean - sem)) + geom_errorbar(size = 2, width  = 0.6, aes(colour = V1)) + geom_point(size = 5, aes(color = V2)) + coord_flip() + ylim(-1,1) + ylab("Cross Correlation") + xlab ("Sample") + scale_color_manual(values = c(vascr_gg_color_hue(3)[2], vascr_gg_color_hue(3)[1], vascr_gg_color_hue(3)[3]))
...
keygraph = p1 + theme(legend.position = "bottom") + 
  ...
  scale_fill_discrete(name = "Treatment", labels = c(expression(paste(" 500 pg/ml TNF",alpha)),expression(paste(" 500 pg/ml IL1",beta)),  "Control"))
